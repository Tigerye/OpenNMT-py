...
def remove_num_comma(input_string):
    pattern = r'\d[\d.，,]+\d'
...
def combine_num_en(input_string):
    scales = ["hundred", "thousand", "million", "billion", "trillion", \
...
def combine_num_zh(input_string):
    scales = ["万", "亿", "千", "百", "兆"]
...
def num_preproc_en(input_string):
    try:
...
def num_preproc_zh(input_string):
    try:
        input_string = remove_num_comma(input_string)
    except:
        input_string = input_string
